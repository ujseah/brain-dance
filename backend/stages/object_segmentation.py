            ...
            with autocast_ctx:
                for frame_idx, obj_ids, mask_logits in self.video_predictor.propagate_in_video(
                    ...
                    for i, obj_id in enumerate(obj_ids):
                        ...
                        # Resize mask to video resolution if needed
                        # SAM-2 may return masks at model resolution when C++ extension unavailable
                        if (
                            self._video_height is not None
                            and self._video_width is not None
                        ):
                            if (
                                mask_h == self._video_height
                                and mask_w == self._video_width
                            ):
                                # Already at video resolution
                                mask = raw_mask.cpu().numpy()
                            else:
                                # Resize from model resolution to video resolution
